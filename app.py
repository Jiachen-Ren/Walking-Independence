...
TRAIN_SKLEARN_VERSION = bundle.get("sklearn_version", "未知")
RUNTIME_SKLEARN_VERSION = sklearn.__version__


if TRAIN_SKLEARN_VERSION != "未知" and TRAIN_SKLEARN_VERSION != RUNTIME_SKLEARN_VERSION:
    st.error(
        "模型文件与当前网页环境的 scikit-learn 版本不一致，可能导致预测时报错。"
        f" 训练时版本：{TRAIN_SKLEARN_VERSION}；当前运行版本：{RUNTIME_SKLEARN_VERSION}。"
        " 请使用模型文件夹内自动生成的 requirements.txt 重新部署。"
    )
...
def predict_probability(raw_df: pd.DataFrame) -> float:
    ...
    for model, preprocessor, imputer in zip(MODELS, PREPROCESSORS, IMPUTERS):
        try:
            ...
            st.error(
                "预测失败：模型文件与当前运行环境的软件版本不兼容。"
                " 请确认部署时使用了本文件夹中的 requirements.txt，尤其 scikit-learn 版本需要与训练时一致。"
            )
            st.exception(exc)
            raise
